...
func (r *ProtocolLXD) CreateContainerFromBackup(data []byte) (Operation, error) {
	// Send the request
	path := "/containers"
	if r.clusterTarget != "" {
		path += fmt.Sprintf("?target=%s", r.clusterTarget)
	}
	op, _, err := r.queryOperation("POST", path, data, "")
	if err != nil {
		return nil, err
	}
...
// API extension: container_backup
func (r *ProtocolLXD) GetContainerBackupFile(containerName string, name string,
